...
export default function () {
  ...
  function convertToNumber(float16bits) {
    ...
    uint32View[0] =
      mantissaTable[offsetTable[m] + (float16bits & 0x3ff)] + exponentTable[m];
    return floatView[0];
  }
  ...
  const isTypedArrayIndexedPropertyWritable = Object.getOwnPropertyDescriptor(
    new Uint8Array(1),
    0
  ).writable;
  ...
  function isDefaultFloat16ArrayMethods(target) {
    return (
      typeof target === "function" && defaultFloat16ArrayMethods.has(target)
    );
  }
  ...
  const handler = {
    get(target, key) {
      ...
      if (isStringNumberKey(key)) {
        return Reflect.has(target, key)
          ? convertToNumber(Reflect.get(target, key))
          : undefined;
      } else {
        const ret =
          wrapper !== null && Reflect.has(wrapper, key)
            ? Reflect.get(wrapper, key)
    ...
    set(target, key, value) {
      ...
      if (isStringNumberKey(key)) {
        ...
        if (
          wrapper !== null &&
          (!Reflect.has(target, key) || Object.isFrozen(wrapper))
        ) {
  ...
  if (!isTypedArrayIndexedPropertyWritable) {
    handler.getPrototypeOf = (wrapper) =>
      Reflect.getPrototypeOf(_(wrapper).target);

    handler.setPrototypeOf = (wrapper, prototype) =>
      Reflect.setPrototypeOf(_(wrapper).target, prototype);

    handler.defineProperty = (wrapper, key, descriptor) => {
      const target = _(wrapper).target;

      return !Reflect.has(target, key) || Object.isFrozen(wrapper)
        ? Reflect.defineProperty(wrapper, key, descriptor)
    ...
    handler.has = (wrapper, key) =>
      Reflect.has(wrapper, key) || Reflect.has(_(wrapper).target, key);
    ...
    handler.getOwnPropertyDescriptor = (wrapper, key) =>
      Reflect.getOwnPropertyDescriptor(wrapper, key);
  ...
  class Float16Array extends Uint16Array {
    constructor(input, byteOffset, length) {
      ...
      if (isFloat16Array(input)) {
        ...
        const arrayLike =
          !Reflect.has(input, "length") && input[Symbol.iterator] !== undefined
            ? [...input]
            : input;
    ...
    *entries() {
    ...
    fill(value) {
      ...
      for (
        var _len3 = arguments.length,
          opts = new Array(_len3 > 1 ? _len3 - 1 : 0),
          _key3 = 1;
        _key3 < _len3;
        _key3++
      ) {
    ...
    copyWithin(target, start) {
      ...
      for (
        var _len4 = arguments.length,
          opts = new Array(_len4 > 2 ? _len4 - 2 : 0),
          _key4 = 2;
        _key4 < _len4;
        _key4++
      ) {
    ...
    sort() {
      ...
      super.sort((x, y) =>
        compareFunction(_convertToNumber(x), _convertToNumber(y))
      );
      return _(this).proxy;
  ...
  function getFloat16(dataView, byteOffset) {
    ...
    for (
      var _len = arguments.length,
        opts = new Array(_len > 2 ? _len - 2 : 0),
        _key = 2;
      _key < _len;
      _key++
    ) {
  ...
  function setFloat16(dataView, byteOffset, value) {
    if (!isDataView(dataView))
      throw new TypeError(
        "First argument to setFloat16 function must be a DataView"
      );

    for (
      var _len2 = arguments.length,
        opts = new Array(_len2 > 3 ? _len2 - 3 : 0),
        _key2 = 3;
      _key2 < _len2;
      _key2++
    ) {
