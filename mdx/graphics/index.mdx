...
主要和CPU,内存打交道. 该阶段末端, 渲染所需的数据(渲染图元rendering primitives)通过<Quote id="#1">数据总线</Quote>传送到GPU
...
3. <Expansion><Quote id="#11">调用Draw Call</Quote></Expansion>: CPU 通知 GPU 渲染
<Tab expan>
    CPU向一个队列"命令缓冲区"push命令, 而GPU从其中pull命令. 这样实现了CPU和GPU的并行工作<br/>
...
<Tab expan>
    ...
    顶点着色器本身本可以创建和销毁任何顶点, 而且<Quote id="#6">顶点顶点之间是相互独立的</Quote>.<br/>
    它最基本且必须完成的工作是: 将顶点坐标从 模型空间 转换到 <Quote id="#7">齐次裁剪坐标系</Quote> 下
</Tab>
2. <Expansion>曲面细分着色器</Expansion>: 用于细分图元(可选着色器, 可编程)
<Tab expan>
    很遗憾, 此部分并没有学到.
</Tab>
3. <Expansion>几何着色器</Expansion>: 执行逐图元的着色(Per-Primitive)或产生更多图元(可选着色器, 可编程)
<Tab expan>
    很遗憾, 此部分并没有学到.
</Tab>
...
<Tab expan>
    确认视锥(即选择投影类型)并裁剪<br/>
    裁剪并不一定是通过GPU来运算. 也可以<Quote id="#8">通过CPU运算</Quote>后直接跳过该步骤.
</Tab>
...
附录: <Expansion><Quote id="#13">坐标空间</Quote></Expansion>: 依照先后顺序
<Tab expan>
    Object space物体坐标系
    <Quote id="#3">--></Quote>
    World space世界坐标系
    ...
    <Table>{
        [{
            cells: ["Object space", "模型建立时得到的坐标. 与其它物体没有任何参照关系"], component: ['th']
        }, {
            cells: ["World space", "以一个固定的坐标原点经行参照确定物体位置"], component: ['th']
        }, {
            cells: ["View space", "以Camera为原点, 组成的正交坐标系"], component: ['th']
        }, {
            cells: ["Project space", "屏幕坐标系"], component: ['th']
        }]
    }</Table>
</Tab>
...
<Tab expan>
    ...
    如果需要得到三角网格对pixel的覆盖情况, 需要计算每条边的像素坐标.<br />
    三角形设置(Triangle Setup)通过计算三角形边界的表示形式来确定每条边的坐标
</Tab>
2. <Expansion>三角形遍历</Expansion>
<Tab expan>
    <Quote id="#9">三角形遍历</Quote>(Triangle Traversal)确定每个三角网格所覆盖的pixel,
    并使用顶点信息对覆盖区域进行差值<br />
    遍历完成后的输出就称之为片元(fragment), 包含了深度值Z, 法线,
    纹理等信息, 可以说是"收集完全但未计算的像素信息"
</Tab>
...
<Tab expan>
    ...
    <br />
    需要经过<Expansion><Quote id="#10">两个测试</Quote></Expansion>,
    来决定片元的可见性问题
    <Tab expan>
    模板测试 --> 深度测试
        <Table>{
            [{
                cells: ["模板测试", "通常用于限制渲染的区域. 也可以用来渲染阴影, 轮廓渲染等"], component: ['th']
            }, {
                cells: ["深度测试", "通常用于视野深度计算"], component: ['th']
            }]
        }</Table>
    </Tab>
    之后需要经行混合(Blend)操作,
