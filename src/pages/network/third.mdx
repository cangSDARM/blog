...
如果 TCP 发送了大量包含少量数据的分组(发送端傻窗口综合症)，网络的性能会严重下降。[Nagle 算法](https://blog.csdn.net/wdscq1234/article/details/52432095)通过在发送一个分组之前，将大量 TCP 数据绑定在一起的方式来提高网络效率。其鼓励发送全尺寸的段，只有全尺寸的被确认后才允许发送非全尺寸的。只有当段被确认，或者缓存中积累了足够发送一个全尺寸段的数据时，才会将缓存的数据发送出去。(有时会有性能问题，可以通过TCP_NODELAY来禁用)
